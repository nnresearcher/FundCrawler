...
from download_fund_info.CrawlingCore import GetPageByWebWithAnotherProcessAndMultiThreading
from download_fund_info.Parser import ParseDefault
from download_fund_info.FundListProvider import GetFundList
from download_fund_info.DataStructure import FundInfo
...
def crawling_fund(fund_list_class: GetFundList, first_crawling=True):
    ...
    crawling_core = GetPageByWebWithAnotherProcessAndMultiThreading(task_queue, result_queue, finish_sign, network_health)
    crawling_core.start()
    #crawling_core.run()
    ...
    while True:
        ...
        while having_fund_need_to_crawl and result_queue.qsize() < 100 and task_queue.qsize() < 10:
            try:
            ...
            task_queue.put(('http://fund.eastmoney.com/' + code + '.html', tem_fund_info))
        # 优先补充输入队列，保证爬取的速度，再处理需要解析的数据\
        while (task_queue.qsize() > 5 or not having_fund_need_to_crawl) and result_queue.qsize():
            a_result = result_queue.get()
            # 若上次的爬取失败了，则重试，未对一直失败的进行排除
            if a_result[0] == 'error':
                task_queue.put(a_result[1:])
            else:
                if a_result[2].next_step == 'parsing_fund':
                    ...
                    #     task_queue.put(('http://fund.eastmoney.com/' + tem_fund_info._fund_info('code') + '.html', tem_fund_info))
                    new_fund_info: FundInfo = fund_web_page_parse.send(a_result[1:])
                    if new_fund_info.next_step == 'parsing_manager':
                        task_queue.put((new_fund_info.manager_need_process_list[-1][0], new_fund_info))
                    else:
        ...
        # 完成所有任务判断
        if not having_fund_need_to_crawl and task_queue.qsize() == 0 and result_queue.qsize() == 0:
            time.sleep(1)
            if not having_fund_need_to_crawl and task_queue.qsize() == 0 and result_queue.qsize() == 0:
                break
